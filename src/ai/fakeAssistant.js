...
export function getFakeAssistantResponse(userInput) {
  ...
  if (/(hostel|accommodation|stay|residence|7|dorm|housing|room|living|accomodation)/.test(lower)) {
    ...
    hostelText += `\n\n ${hostelData.status ? "I've opened the Hostel page where you can explore all facilities and start the interactive tour." : "Coming Soon"}`;

    return {
      text: hostelText,
      navigateTo: hostelData.status ? '/hostel' : '/',
  ...
  if (/(tour|audio tour|voice tour|guided tour|interactive tour)/.test(lower) && lower.includes('hostel')) {
    return {
      text:
       ...
       `${hostelData.status ? "I've opened the Hostel page where you can experience this interactive tour!" : "Coming Soon"}`,
